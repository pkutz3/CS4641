...
class SVDRecommender(object):
    ...
    def recommender_svd(
        ...
        ic = ImgCompression()
        U, S, V = ic.svd(R)
        Uc, Sc, Vc = ic.compress(U,S,V,k)
        Uk = np.matmul(Uc,np.diag(Sc)**.5)
        Vk = np.matmul(np.diag(Sc)**.5,Vc)
        return Uk,Vk
    ...
    def predict(
        ...
        """
        idx = users_index[user_id]
        notseen = []
        movie_ratings = {}
        recommendation = []
        for movie in movies_pool:
            mid = self.get_movie_id_by_name(movie)
            midx = movies_index[mid]
            if np.isnan(R[idx,midx]):
                notseen.append((movie,midx))
        for i,movie in enumerate(notseen):
            v = V_k[:,notseen[i][1]]
            u = U_k[idx]
            rating = np.dot(u,v)
            movie_ratings[rating] = movie
        sorted_movies = list(movie_ratings.keys())
        sorted_movies.sort(reverse=True)
        for rating in sorted_movies[:top_n]:
            recommendation.append(movie_ratings[rating][0])
