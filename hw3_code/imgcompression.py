...
class ImgCompression(object):
    ...
    def compress(
        ...
        D = V.shape[0]
        if U.ndim == 2:
            #create compressed up to k based on expected shapes above
            U_comp = U[:,:k]
            S_comp = S[:k]
            V_comp = V[:k,:]
        else:
            U_comp = np.zeros((N,k,3))
            S_comp = np.zeros((k,3))
            V_comp = np.zeros((k,D,3))

            #create compressed for each RGB
            for i in range(3):
                U_comp[:,:,i] = U[:,:k,i]
                S_comp[:,i] = S[:k,i]
                V_comp[:,:,i] = V[:k,:,i]
    ...
    def rebuild_svd(
        ...
        if U_compressed.ndim == 2:
            #U * diag(S) * V
            SV = np.matmul(np.diag(S_compressed),V_compressed)
            Xrebuild = np.matmul(U_compressed,SV)
        else:
            Xrebuild = np.zeros((N,D,3))
            #U * diag(S) * V for each RGB
            for i in range(3):
                SV = np.matmul(np.diag(S_compressed[:,i]),V_compressed[:,:,i])
                Xrebuild[:,:,i] = np.matmul(U_compressed[:,:,i],SV)
        return Xrebuild
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        """
        #variance from notebook
        return (S[:k]**2).sum(axis=0) / (S**2).sum(axis=0)
        #raise NotImplementedError
