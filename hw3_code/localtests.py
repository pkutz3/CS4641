...
from local_tests_folder.svd_recommender_test import SVDRecommender_Test
from local_tests_folder.lr_test import LogisticRegression_Test
...
class TestImgCompression(unittest.TestCase):
    ...
    def test_compress_bw(self):
        ...
        # check compression for U, S, V
        self.assertEqual(
            np.allclose(Uc, test_ic.Ugc), True, "U compression is incorrect"
        )
        self.assertEqual(
            np.allclose(Sc, test_ic.Sgc), True, "S compression is incorrect"
        )
        self.assertEqual(
            np.allclose(Vc, test_ic.Vgc), True, "V compression is incorrect"
        )
    ...
    def test_rebuild_svd_bw(self):
        ...
        self.assertEqual(
            np.allclose(Xrebuild_g, test_ic.Xrebuild_g),
            True,
    ...
    def test_compress_color(self):
        ...
        # check compression for U, S, V
        self.assertEqual(
            np.allclose(Uc, test_ic.Ucc), True, "U compression is incorrect"
        )
        self.assertEqual(
            np.allclose(Sc, test_ic.Scc), True, "S compression is incorrect"
        )
        self.assertEqual(
            np.allclose(Vc, test_ic.Vcc), True, "V compression is incorrect"
        )
    ...
    def test_rebuild_svd_color(self):
        ...
        # check reconstruction
        self.assertEqual(
            np.allclose(Xrebuild_c, test_ic.Xrebuild_c),
            True,
            "Reconstruction is incorrect",
        )
    ...
    def test_compression_ratio_color(self):
        ...
        # check compression ratio
        self.assertEqual(
            np.allclose(cr, test_ic.cr_c), True, "Compression ratio is incorrect"
        )
...
class TestSVDRecommender(unittest.TestCase):
    ...
    def test_recommender_svd(self):
        ...
        correct_slice_U_k, correct_slice_V_k = (
            test_recommender.slice_U_k,
            test_recommender.slice_V_k,
        )

        self.assertTrue(
            np.all(U_k.shape == test_recommender.U_k_expected_shape),
            "recommender_svd() function returning incorrect U_k shape",
        )

        self.assertTrue(
            np.all(V_k.shape == test_recommender.V_k_expected_shape),
            "recommender_svd() function returning incorrect V_k shape",
        )

        self.assertEqual(
            np.allclose(my_slice_U_k, correct_slice_U_k),
            True,
            "recommender_svd() function returning incorrect U_k",
        )
        self.assertEqual(
            np.allclose(my_slice_V_k, correct_slice_V_k),
            True,
    ...
    def test_predict(self):
        ...
        movie_recommendations = recommender.predict(
            R,
            U_k,
            V_k,
            users_index,
            movies_index,
            test_recommender.test_user_id,
        ...
        self.assertEqual(
            len(movie_recommendations)
            == len(test_recommender.predict_expected_outputs),
            True,
            "predict() function is not returning the correct number of recommendations",
        )

        self.assertEqual(
            (movie_recommendations == test_recommender.predict_expected_outputs),
            True,
...
class TestPCA(unittest.TestCase):
    ...
    def test_transform_rv(self):
        ...
        # check transformed data
        self.assertEqual(
            np.allclose(X_new_rv, test_pca.X_new_rv),
            True,
...
class TestRegression(unittest.TestCase):
    ...
    def test_predict(self):
        ...
        predict_test = np.allclose(
            reg.predict(test_reg.x_all_feat, test_reg.true_weight), test_reg.predict
        )
    ...
    def test_linear_fit_closed(self):
        ...
        linear_closed_test = np.allclose(
            reg.linear_fit_closed(test_reg.x_all_feat, test_reg.y_all),
            test_reg.linear_closed,
            rtol=1e-04,
        )
...
class TestLogisticRegression(unittest.TestCase):
    ...
    def test_fit(self):
        ...
        result = lr.fit(
            test_lr.x, test_lr.y, test_lr.x, test_lr.y, test_lr.lr, test_lr.epochs
        )
