...
class LogisticRegression(object):
    ...
    def bias_augment(self, x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        """
        return np.concatenate((np.array([1 for i in range(x.shape[0])])[:, np.newaxis], x), axis=1)
        #raise NotImplementedError
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float: # [3pts]
        ...
        Return:
            loss (float)
        """
        preSum = y*np.log(h_x)+(1-y)*np.log(1-h_x)
        return -np.sum(preSum)/y.shape[0]
        #raise NotImplementedError
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        """
        return np.matmul(x_aug.T,h_x-y)/y.shape[0]
        #raise NotImplementedError
    ...
    def accuracy(self, y: np.ndarray, y_hat: np.ndarray) -> float: # [2pts]
        ...
        """
        return np.sum((y==y_hat).astype(int))/y.shape[0]
        #raise NotImplementedError
