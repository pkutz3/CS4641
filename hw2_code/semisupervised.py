...
def incomplete_(data): # [1pts]
    ...
    """    
    with_labels = data[~np.isnan(data[:,-1])]
    return with_labels[np.isnan(with_labels).any(axis=1)]
    #raise NotImplementedError
...
def unlabeled_(data): # [1pts]
    ...
    """
    wo_labels = data[np.isnan(data[:,-1])]
    return wo_labels[~np.isnan(wo_labels[:,:-1]).any(axis=1)]
    #raise NotImplementedError
...
def mean_clean_data(data): # [2pts]
    ...
    """
    mean_clean = np.zeros(data.shape)
    for i, row in enumerate(data):
        if np.isnan(row).any():
            nanIndex = np.argwhere(np.isnan(row))[0][0]
            row[nanIndex] = np.round(np.nanmean(data[:,nanIndex]),1)
        mean_clean[i] = row
    return mean_clean
    #raise NotImplementedError
